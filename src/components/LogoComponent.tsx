...
export default function LogoComponent({ 
  ...
  // Cargar logo corporativo desde Imgur
  ...
  return (
    <div className={`flex items-center ${className}`}>
      {/* Logo corporativo - Múltiples fuentes */}
      <img 
        ...
        onLoad={() => {
          console.log('🎉 ¡LOGO CORPORATIVO HUMANBIBLIO CARGADO!');
          console.log('🎨 Logo corporativo funcionando perfectamente');
          console.log('🏛️ HUMANBIBLIO con branding corporativo activado');
        }}
        style={{ maxHeight: '100%', width: 'auto', filter: 'drop-shadow(0 10px 25px rgba(0, 0, 0, 0.1))' }}
      />
      
      {/* Fallback emoji profesional (oculto por defecto) */}
      <div 
        ...
        onClick={onClick}
        onDoubleClick={onDoubleClick}
        style={{ fontSize: size === 'hero-large' ? '6rem' : size === 'large' ? '8rem' : size === 'hero' ? '3rem' : size === 'medium' ? '2rem' : '1.5rem' }}
      >
        <div className="bg-gradient-to-br from-blue-600 to-purple-600 text-white rounded-2xl p-6 shadow-2xl border-2 border-white/20 backdrop-blur-sm animate-pulse">
          <div className="text-center">
            ...
            <div className={`opacity-90 ${size === 'hero-large' ? 'text-base' : 'text-sm'}`}>Logo Corporativo</div>
            <div className={`opacity-75 ${size === 'hero-large' ? 'text-sm' : 'text-xs'} mt-1`}>🌍 Global • 🗣️ Multicultural</div>
          </div>
        </div>
      </div>
    </div>
